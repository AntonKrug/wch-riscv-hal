...
namespace soc::gpio {
    ...
    struct Op { // NOLINT
        ...
        std::uint32_t bit_alias_address; // alternative address to use to apply set/reset mechanism
        ...
        std::uint32_t reset_bits;        // how many bits the reset_value takes
    };
    ...
    constexpr void execute_op() {
        if constexpr (TplOp.writable == TplOp.mask) {
        ...
        } else  if constexpr (TplOp.bit_alias_address != 0U) {
            ...
            soc::reg::access::writeCtAddrVal<TplOp.bit_alias_address, bit_set_clear_value>();
        } else {
